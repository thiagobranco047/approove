...
interface CalendarModalProps {
  ...
  posts: Post[];
  onDayClick: (postIndex: number, date?: Date) => void;
  currentMonth: Date;
  isAdmin?: boolean;
}
...
export function CalendarModal({
  ...
  const handleDayClick = (day: Date) => {
    ...
    if (dayPosts.length > 0) {
      const firstPostIndex = posts.findIndex((p) => p.id === dayPosts[0].id);
      onDayClick(firstPostIndex);
      onOpenChange(false);
    } else if (isAdmin) {
      onDayClick(-1, day);
      onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl" data-component="calendar-modal" id="calendar-modal">
        <DialogHeader>
        ...
        {/* Status Legend */}
        <div className="flex items-center justify-center gap-4 text-xs text-muted-foreground" data-section="status-legend">
        ...
        <div className="grid grid-cols-7 gap-2" data-section="calendar-grid">
          ...
          {calendarDays.map((day, index) => {
            ...
            const canCreateEmpty = isAdmin && isEmpty && isCurrentMonth;
            const canAddMore = isAdmin && !isEmpty && isCurrentMonth;
            ...
            return (
              <Button
                ...
                className={`group h-16 p-1 flex flex-col items-center justify-center relative ${
                  ...
                  canCreateEmpty ? "hover:border-primary hover:border-dashed hover:bg-primary/5" : ""
                }`}
              ...
              >
                <span className={`text-sm ${canCreateEmpty ? "group-hover:hidden" : ""}`}>
                  {format(day, "d")}
                </span>
                ...
                {dayPostsCount > 0 && (() => {
                  ...
                  const adjustments = dayPosts.filter((p) => p.status === "adjustments").length;
                  const badges: { count: number; color: string; label: string }[] = [];
                  if (approved > 0) badges.push({ count: approved, color: "bg-green-500", label: "Aprovado" });
                  if (pending > 0) badges.push({ count: pending, color: "bg-gray-500", label: "Pendente" });
                  if (adjustments > 0) badges.push({ count: adjustments, color: "bg-orange-500", label: "Ajustes" });

                  return (
                    <div className="flex items-center gap-0.5 mt-0.5" data-section="status-badges">
                      {badges.map((b) => (
                        <span
                          key={b.label}
                ...
                {canCreateEmpty && (
                  <div className="hidden group-hover:flex flex-col items-center gap-0.5">
                    ...
                    <span className="text-[10px] text-primary font-medium leading-tight">Criar</span>
                  </div>
                )}
                ...
                {canAddMore && (
                  <div
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter") handleAddPostToDay(e as unknown as React.MouseEvent, day); }}
